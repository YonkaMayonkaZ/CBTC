...
Σύντομη απάντηση: **ναι, ο βασικός CBTC επαληθεύεται** και από τα γραφήματα και από τους
αριθμούς ενέργειας. Το shrink-back δίνει τη μεγάλη εξοικονόμηση που περιμένουμε, αλλά η
απλοποιημένη υλοποίησή του έχει ένα όριο που σχολιάζουμε παρακάτω.
...
### 4.4 Shrink-back — εξοικονόμηση ενέργειας και ένα όριο της υλοποίησης

Το shrink-back δίνει **μεγάλη εξοικονόμηση ενέργειας**: π.χ. `two_clusters` στις 150° πέφτει
από **8 801 → 2 381** (≈ −73%), `random100` από **25 540 → 15 157** (≈ −41%). Αυτό
επιβεβαιώνει τον ρόλο της Φάσης 2.

**Όμως** η απλοποιημένη υλοποίησή μας είναι **υπερβολικά επιθετική** σε δίκτυα που κυριαρχούν
κόμβοι συνόρου: το `ring` με shrink **αποσυνδέεται** (βαθμός 1.25, 3 συνιστώσες), και κάποιες
περιπτώσεις στις 120° (`random100`, `two_clusters`) επίσης. Ο λόγος: για κόμβους σε κυρτή θέση
υπάρχει **πάντα** ένα μη-καλύψιμο κενό, οπότε ο κανόνας «αφαίρεσε εφόσον δεν ανοίγει νέο κενό»
αφαιρεί σχεδόν τα πάντα.

![ring με shrink-back](figures/ring_alpha150_shrink.png)

*Ο δακτύλιος «σπάει»: από 16 ακμές (κλειστός κύκλος) σε 5 ασύνδετα τμήματα.*

Αυτό **δεν αναιρεί** τη θεωρία — το shrink-back του paper διατηρεί τη συνεκτικότητα γιατί
κρατά **ακριβώς** το σύνολο κώνων που καλυπτόταν στη μέγιστη ισχύ και συνδυάζεται με τα άλλα
βήματα της Φάσης 2. Είναι **όριο της δικής μας απλοποιημένης έκδοσης**, και γι' αυτό το
shrink-back πρέπει πάντα να συνοδεύεται από τον έλεγχο συνεκτικότητας (που ο κώδικας ήδη
τυπώνει). Μια πιστή υλοποίηση απαιτεί τον αυστηρότερο κανόνα «διατήρησε τους ίδιους κώνους».
...
- **Το shrink-back** δίνει την αναμενόμενη μεγάλη εξοικονόμηση ενέργειας, αλλά η απλοποιημένη
  υλοποίηση χρειάζεται τον έλεγχο συνεκτικότητας ως δικλείδα ασφαλείας (όριο, όχι αντίφαση).
